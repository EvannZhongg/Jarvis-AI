...
import { get, type ModelOption, type ModelOptions, type Session, type SessionSummary } from "./api";
...
export function App() {
  ...
  async function selectSession(id: string) {
    ...
    setError("");
    try {
      setSession(await get<Session>(`/api/sessions/${encodeURIComponent(id)}`));
    } catch (error) {
      setError(String(error));
    } finally {
      setLoading(false);
    ...
    <div className="app-shell">
      ...
      <main className="chat-panel">
        ...
        <Chat key={session.session_id} session={session} disabled={loading || !model}
          models={models} model={model} onModelChange={setModel} onBusyChange={setBusy} onDone={() => {
